...
def test_tx_delete_success(client, auth_user, added_transaction):
    ...
    response = client.delete(f"/transactions/{added_transaction["id"]}", headers=headers)
    assert response.status_code == 200
    data = response.get_json()
    assert data["message"] == f"Transaction with id: {added_transaction["id"]}, deleted successfully!"
