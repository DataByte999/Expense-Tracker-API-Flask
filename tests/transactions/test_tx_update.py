...
def test_tx_update_success(client, auth_user, added_transaction):
    ...
    response = client.patch(
        f"/transactions/{added_transaction['id']}", json=update_fields, headers=headers
    )
...
def test_tx_update_transaction_not_found(client, auth_user, tx_payload):
    ...
    response = client.patch(
        f"/transactions/{nonexistent_tx_id}", json=tx_payload, headers=headers
    )
...
def test_tx_update_empty_payload(client, auth_user, added_transaction):
    ...
    _user, headers = auth_user
    response = client.patch(
        f"/transactions/{added_transaction['id']}", json={}, headers=headers
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_tx_update_amount_invalid_type(
    client, auth_user, added_transaction, tx_payload
):
    """Should return 400 when payload data type is invalid"""
    _user, headers = auth_user
    tx_payload["amount"] = "One hundred"
    response = client.patch(
        f"/transactions/{added_transaction['id']}", json=tx_payload, headers=headers
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
