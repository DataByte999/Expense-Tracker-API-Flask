...
from tests.factories import make_token, make_transaction, make_user
...
def tx_payload(faker):
    ...
    return {
        ...
        "amount": str(
            fake.pydecimal(left_digits=3, right_digits=2, positive=True, min_value=1)
        ),
...
def added_transaction(auth_user):
    ...
    user = auth_user[0]
    return make_transaction(user_id=user["id"])
